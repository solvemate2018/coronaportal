...
public class TestController {
    ...
    public String testCenter(Model model, Principal principal){
        ...
        List<userViewTestAppointmentsViewModel> modelView = new ArrayList<userViewTestAppointmentsViewModel>();
        for (TestAppointment testApp: testAppointments) {
            ...
            if(result == null){
                modelView.add(new userViewTestAppointmentsViewModel(testApp.getId(), testApp.getTest_time(), center.getCity(), LocalDateTime.now(), "0"));
            }
            else {
                modelView.add(new userViewTestAppointmentsViewModel(testApp.getId(), testApp.getTest_time(), center.getCity(), result.getTime_of_result(), result.getResult()));
            }
        }
        modelView.sort(Comparator.comparing(userViewTestAppointmentsViewModel::getTestTime).reversed());
